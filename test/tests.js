const IdentityHandler = require("../src/identity-handler");
...
describe("IdentityHandler", () => {
  it("on identify it should track a heartbeat", (done) => {
    const upClient = new StubUpClient();
    const handler = new IdentityHandler({ upClient: upClient });

    handler.onIdentifyComplete({
      getUserIdentities: () => ({ userIdentities: { email: "foo@bar.com" } }),
    });

    expect(upClient.userInfo.userEmail).to.equal("foo@bar.com");
    expect(upClient.eventType).to.equal(16);

    done();
  });

  it("Identify with no details should do nothing", (done) => {
    const upClient = new StubUpClient();
    const handler = new IdentityHandler({ upClient: upClient });
    ...
    expect(upClient.userInfo).to.equal(undefined);
    done();
  });

  it("on login it should track a login", (done) => {
    const upClient = new StubUpClient();
    const handler = new IdentityHandler({ upClient: upClient });

    handler.onLoginComplete({
      getUserIdentities: () => ({
        userIdentities: {
          email: "foo@bar.com",
          customerid: "u_123",
          mobile_phone: "+641112222",
        },
      }),
    });

    expect(upClient.userInfo.userEmail).to.equal("foo@bar.com");
    expect(upClient.userInfo.userId).to.equal("u_123");
    expect(upClient.userInfo.userPhone).to.equal("+641112222");
    expect(upClient.eventType).to.equal(1);
